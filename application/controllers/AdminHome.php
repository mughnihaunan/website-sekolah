...
class AdminHome extends CI_Controller
{
		...
		if (isset($_POST['submit'])) {
			...
			if (!empty($_FILES['file_image']['name'])) {
				...
				if ($this->upload->do_upload('file_image')) {
					...
					$urlImgUtama = base_url('assets/uploads/berita/' . $upload_data['file_name']);
				} else {
				...
				$urlImgUtama = $this->input->post('urlImgUtama', true);
			}
			...
			if (!$this->form_validation->run()) {
				...
				// Insert berita with image URL
				$data = [
					'berita_id' => $this->uuid->generate_uuid(),
					'judulBesar' => $this->input->post('judulBesar', true),
					'judulKecil' => $this->input->post('judulKecil', true),
					'urlImgUtama' => $urlImgUtama,
	...
	public function editBerita()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$berita_id = $this->input->post('berita_id', true);
			$urlImgUtama = '';
			...
			if (!empty($_FILES['file_image']['name'])) {
				...
				if ($this->upload->do_upload('file_image')) {
					...
					$this->session->set_flashdata('error', $this->upload->display_errors());
					redirect('adminHome/editBerita/' . $berita_id);
					return false;
				}
			} else {
				$urlImgUtama = $this->input->post('urlImgUtama', true);
